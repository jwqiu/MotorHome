...
Purpose:  Whodat? looks at cgi.http_user_agent to determine what browser the user 
		...
		visitor's browsers which will hopefully allow the web-developer to build a 
...
Returns: browserdata = cgi.http_user_agent (The user agent string)
		MozillaVersion = The Mozilla version, or "other" if Mozilla version is not applicable (as in Lynx)
		...
		============================
		JSVersion and EcmaCompliant will automatically return "no" if the browser is not Javascript Friendly.
		============================
...
If you have suggestions for improving this tag please send them to me and I'll 
...
file. Please refrain from sending me addresses that would direct users to any 
"adult" site.

Check out the Salt Lake City, Utah ColdFusion users group site at http://www.slcfug.org
--->
...
if (Mozilla NEQ 0 and MSIE EQ 0 and Opera EQ 0 and WebTV EQ 0 and Lynx EQ 0 and AOL EQ 0 and compatible EQ 0){
...
else if (MSIE NEQ 0 and Opera EQ 0 and WebTV EQ 0 and Lynx EQ 0 and AOL EQ 0){
...
else if ((FindNoCase("os/2", browserdata) NEQ 0) OR (FindNoCase("ibm-webexplorer", browserdata) NEQ 0)){
	Platform="OS/2";
}
...
//DCSS friendly?
if ((BrowserName EQ "Netscape" AND BrowserVersion GTE 4) OR (BrowserName EQ "Microsoft" AND BrowserVersion GTE 4) OR (BrowserName EQ "AOL" AND BrowserVersion GTE 3) OR (BrowserName EQ "Opera" AND BrowserVersion GTE 3.6) OR (BrowserName EQ "WebTV")){
	CssFriendly="Yes";
}

//DHTML friendly?
if ((BrowserName EQ "Netscape" AND BrowserVersion GTE 4) OR (BrowserName EQ "Microsoft" AND BrowserVersion GTE 4) OR (BrowserName EQ "AOL" AND BrowserVersion GTE 4)){
	DhtmlFriendly="Yes";
}

//XML friendly?
if ((BrowserName EQ "Netscape" AND BrowserVersion GTE 6) OR (BrowserName EQ "Microsoft" AND BrowserVersion GTE 5) OR (BrowserName EQ "Opera" AND BrowserVersion GTE 4)){
	XmlFriendly="Yes";
}

//JAVA friendly?
if ((BrowserName EQ "Netscape" AND BrowserVersion GTE 3) OR (BrowserName EQ "Microsoft" AND BrowserVersion GTE 3) OR (BrowserName EQ "AOL" AND BrowserVersion GTE 3)){
	JavaFriendly="Yes";
}


//Javascript friendly? What version? ECMA compliant?
if ((BrowserName EQ "Netscape" AND BrowserVersion GTE 2) OR (BrowserName EQ "Microsoft" AND BrowserVersion GTE 3) OR (BrowserName EQ "AOL" AND BrowserVersion GTE 3) OR (BrowserName EQ "Opera" AND BrowserVersion GTE 3.5) OR (BrowserName EQ "WebTV")){
...
//Set Callers
CALLER.browserdata="#browserdata#";
