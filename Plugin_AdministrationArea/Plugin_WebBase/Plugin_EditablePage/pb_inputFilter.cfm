<CFSILENT>
	<!--------------------------------------------------------------------------
		...
		<CFIF NOT DirectoryExists(ExpandPath('Content/'))>
			<CFDIRECTORY ACTION="CREATE" DIRECTORY="#ExpandPath('Content/')#">
			<CFFILE ACTION="WRITE" FILE="#ExpandPath('Content/Application.cfm')#" 
				OUTPUT="<CFSILENT> <!--- WIBBLE ! ---> </CFSILENT>"> 
		</CFIF>
		
		<CFIF NOT ListFindNoCase("Services,ResetServices,DumpServices,Return,#CFG.PlName#Include", ATTRIBUTES.FuseAction)>
			<CFIF pb_serviceAvailable("authenticate")>
