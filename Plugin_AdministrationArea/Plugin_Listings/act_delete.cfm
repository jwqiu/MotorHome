<CFSILENT>
	...
	<CFELSE>	
		...
		<!--- We must also delete from any multilookup or oldimage fields --->
		<CFLOOP QUERY="CFG.Fields">
			...
			<CFIF Type eq "oldimage">
				<CFQUERY DATASOURCE="#CFG.DS#" DBTYPE="#CFG.DBTYPE#" CONNECTSTRING="#CFG.CONNECTSTRING#" NAME="Q_DelLookups">
					SELECT #FieldName# FROM #CFG.PlTableName#
						WHERE #CFG.PlTablePrimaryKey# IN (#ATTRIBUTES[CFG.PlTablePrimaryKey]#)
				</CFQUERY>
				<CFMODULE TEMPLATE="safeDelete.cfm" FILE="#ExpandPath('#Directory#/#Evaluate('Q_DelLookups.#FieldName#')#')#">
				<CFMODULE TEMPLATE="safeDelete.cfm" FILE="#ExpandPath('#Directory#/imThumbs/_#Evaluate('Q_DelLookups.#FieldName#')#')#">
			</CFIF>
		</CFLOOP>
