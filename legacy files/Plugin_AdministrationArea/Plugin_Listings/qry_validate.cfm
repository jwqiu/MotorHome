<CFSILENT>
	...
	<!---
		- Takes validates the attributes given it that match to 
		- fields this plugin is handling, we validate these into a 
		- structure called ATTRIBUTES.Validated (as we may modify
		- values as we are validating)
		- we also place a list of errors that happened into 
		- ATTRIBUTES.Validated.entryErrors
		- and return the validated attributes as a WDDX 
		- --->
	...
	<CFSET ATTRIBUTES.Validated = StructNew()>
	...
	<CFLOOP QUERY="CFG.Fields">
		...
		<CFSWITCH EXPRESSION="#Type#">
			...
			<CFCASE VALUE="oldimage">
				<CFMODULE TEMPLATE="image.cfm" METHOD="processAction" 
					  FieldName="#FieldName#" ImagePath="#Directory#">	
				<CFSET Value="#Evaluate('ATTRIBUTES.#FieldName#')#">
			</CFCASE>
		</CFSWITCH>
		...
		<!--- copy field  into attributes.validated --->
		<CFSET ATTRIBUTES.Validated[FieldName] = Value>
	</CFLOOP>
	
	<!--- copy attributes.entryErrors into attributes.validated.entryErrors --->
	<CFSET ATTRIBUTES.Validated.entryErrors = ATTRIBUTES.entryErrors>
	<CFSET ATTRIBUTES.Validated.errorFields = ATTRIBUTES.errorFields>
	</CFIF>
</CFSILENT>